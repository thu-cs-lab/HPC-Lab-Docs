...
cd ~/PA4_build
spack load cuda
cmake ../PA4 # 会自动clone Googletest，可能会因为网络问题很慢，可以尝试 cp -r /home/course/hpc/assignments/2021/googletest/ ~/PA4_build 来解决
make -j
srun ./test/unit_tests --dataset <datasetname> --len 32 --datadir <where you put your data, optional>  # 在PA4/script目录下也有脚本可以使用
```
