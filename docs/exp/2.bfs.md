...
1. 正确性（$60 \%$）
    * 正确性得分包含两部分：基础分（占10%）和加速比得分（占50%），对于每组测试用例，只有当你获得了正确性基础分数后，才能得到加速比得分。
      * 对于基础分，助教可能使用符合要求的 **任意线程数与进程数** 来运行你的程序，如果你的代码运行结果与串行代码（即`bfs_serial`）运行结果相同，就可以获得该测试用例的基础分。
      * 对于加速比得分，由于不同实现在不同情况下的性能表现可能不同，此部分最终的运行方式由同学确定，**OpenMP由同学自行设置线程数（以及线程绑定等），MPI至多可以使用 4 机 112 进程** 。我们提供了 `run_bfs_omp.sh` 和 `run_bfs_mpi.sh`，**请同学修改脚本中的运行命令和环境变量，助教将使用此作为最终的加速比得分的评分依据**（运行方式如 `./run_bfs_omp.sh input_file`）。加速比测试只针对`bfs_serial`, `bfs_omp` 和 `bfs_omp_mpi` 函数，即仅对这三个函数计时（其中，`bfs_serial`由助教提供）。
      * 对于OpenMP程序，**在`68m.graph`图下 `bfs_omp` 函数相对于`bfs_serial`函数的加速比超过2**，就可以获得该测试用例的加速比得分；
      * 对于OpenMP+MPI程序，**在`68m.graph`图下 `bfs_omp_mpi` 函数相对于`bfs_omp`函数的加速比超过1.2**，就可以获得该测试用例的加速比得分。
    * 有多组组测试用例。你将获得通过的测试用例的分数，每个测试用例分数相同。
2. 性能（$30 \%$）
    * 对于每组测试用例，只有当你获得了正确性基础分数后，才能得到性能分。每组测试用例的性能分数相同。
    * 助教将使用你提供的 `run_bfs_omp.sh` 和 `run_bfs_mpi.sh`作为最终的性能评分依据。
