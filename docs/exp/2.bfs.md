...
3. 可以尝试将进程/线程与核绑定（ **尤其是 OpenMP+MPI 混合时** ），运行性能会更稳定（具体可阅读集群使用手册的 SLURM 使用部分）。一般来说，推荐选取 rank 数量与 NUMA 节点数量相同（实验集群上为每机 2 进程），每个 rank 的线程数量与单个 NUMA 节点物理核心数量相同（实验集群上为每进程 14 线程），并将每个线程绑定到对应的物理核心上。
