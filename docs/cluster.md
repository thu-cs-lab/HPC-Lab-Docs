...
在单独使用 OpenMP 时，推荐将每个线程绑定到单个 CPU 核心（使用 `OMP_PROC_BIND=close OMP_PLCES=cores` 环境变量），以获得最佳性能。关于 OpenMP 线程绑定的更详细说明，可见 [OpenMP topic: Affinity](https://pages.tacc.utexas.edu/~eijkhout/pcse/html/omp-affinity.html)。
