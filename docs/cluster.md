...
集群的登录节点 `conv0` 可供 SSH 访问，通过下发的用户名（学号）和初始密码登录到 `166.111.236.65` 的 `22222` 端口即可。在校内和校外都可直接访问，无需使用代理或者 VPN。如果使用终端，命令类似 `ssh -p 22222 2019000000@166.111.236.65`。如果使用客户端，注意不要忘记配置端口和用户名。
