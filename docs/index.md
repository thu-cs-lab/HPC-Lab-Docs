...
* [实验一](exp/1.odd_even_sort.md)（奇偶排序）：基于消息传递的并行编程（MPI）
* [实验二](exp/2.apsp.md)（全源最短路）：CUDA 编程
...
* 本学期课程助教：
    * 张闰清：zhangrq24@mails.tsinghua.edu.cn (总协调)
    * 杨恺：yangkai21@mails.tsinghua.edu.cn（实验集群）
* 课程建设：
    * 陈晟祺（实验环境）
